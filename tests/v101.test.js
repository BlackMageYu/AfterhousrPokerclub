...
test('fast fold cancels a pending external decision and settles without waiting for the API',async t=>{
  ...
  let started=false;const service=new AIGameService(store,settings,{request:(_config,_observation,{signal})=>new Promise((resolve,reject)=>{started=true;signal.addEventListener('abort',()=>reject(new AIError('cancelled','cancelled')),{once:true});})});
  store.dispatch('start',{config:{seats:5}});assert.notEqual(store.session.engine.hand.actor,0);const pending=service.dispatch('tick',turn(store));
  await new Promise(resolve=>setImmediate(resolve));assert.equal(started,true);
  const result=await service.dispatch('action',{...turn(store),action:'fold',fastFold:true});
