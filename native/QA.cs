...
public partial class MainWindow {
 ...
 async Task RunQA(){try{
  ...
  QARequire(Hand["status"].S=="complete","hand must complete");Render();QARequire(dialogCount==0,"hand settlement must not automatically open compact review");QARequire(animationLayer!=null&&animationLayer.Children.Count>0,"settlement creates the chip-payout animation layer");await Shot("05-compact");HideOverlay();state=await rules.Call("end",Turn("reason","原生端到端验收"));Render();ShowSummary();await Shot("06-summary");HideOverlay();await ShowReplay();await Task.Delay(700);SetReplay(4);await Shot("07-replay");SetReplay(int.MaxValue);revealCards.IsChecked=true;SetReplay(replayIndex);await Shot("08-replay-final");HideOverlay();await ShowContacts();await Shot("09-contacts");HideOverlay();int completed=state["lastSummary"]["completedHands"].I;
