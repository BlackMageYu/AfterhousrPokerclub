...
public partial class MainWindow {
 ...
 async Task RunQA(){try{
  ...
  if(!Hand["actor"].Null&&Hand["actor"].I!=0){state=await rules.Call("tick",Turn("botTiming",true));Render();if(!Hand["botTiming"].Null){double started=Hand["botTiming"]["startedAt"].N;UpdateAvatarClock(started);QARequire(countdownSeat==Hand["actor"].I&&avatarCountdown.Text=="10","bot countdown always begins at the private ten-second display");UpdateAvatarClock(started+7000);QARequire(avatarCountdown.Text=="3","bot display countdown never exposes its real think time");await Shot("10-bot-countdown");}}
  int steps=0;while(Hand["actor"].I!=0&&Hand["status"].S=="playing"&&steps++<30)state=await rules.Call("tick",Turn());
  Render();await CheckTableInteractions();ShowBank();UpdateLayout();int bankSliders=VisualCount<Slider>(overlay),bankInputs=VisualCount<TextBox>(overlay);QARequire(dialogCount==1&&bankSliders==2&&bankInputs==0,"chip management uses two slider controls without amount text fields (sliders="+bankSliders+", inputs="+bankInputs+")");await Shot("13-bank-sliders");HideOverlay();ShowGameConfirm("退出游戏","确定要退出 AfterHours 吗？\n当前进度会被安全保存。","退出",()=>{});QARequire(dialogCount==1,"game-styled confirmation dialog opens inside the overlay");await Shot("14-game-confirm");HideOverlay();await ShowSettings();await Shot("03-settings");HideOverlay();ShowPlayer(Session["players"][1]);await Shot("04-profile");HideOverlay();
  ...
  File.WriteAllText(Path.Combine(root,"qa","result.json"),J.Json(J.O("ok",true,"hands",completed,"wallet",state["profile"]["wallet"].N,"renderer","WPF PresentationCore","transport","private child process stdio","uiChecks",new[]{"avatar-countdown","private-10-second-bot-countdown","room-remaining-time","shrinking-arc","pause-resume","timebank-button","raise-amount-sync","stack-clamp","bank-sliders","game-styled-confirmation","no-auto-compact-review","chip-payout-animation","2/3/5/7-seat-layouts","16:9-fill","compact-and-detailed-replay"})));Close();
