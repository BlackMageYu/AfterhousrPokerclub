...
public sealed class RulesClient : IDisposable {
 Process process; SemaphoreSlim gate=new SemaphoreSlim(1,1); int sequence; public string Root;
 public RulesClient(string root,string dataRoot){Root=root;var p=new ProcessStartInfo(System.IO.Path.Combine(root,"runtime","rules.exe"),"\""+System.IO.Path.Combine(root,"native","bridge.mjs")+"\"");p.WorkingDirectory=root;p.UseShellExecute=false;p.CreateNoWindow=true;p.RedirectStandardInput=true;p.RedirectStandardOutput=true;p.RedirectStandardError=true;p.StandardOutputEncoding=Encoding.UTF8;p.StandardErrorEncoding=Encoding.UTF8;if(!string.IsNullOrEmpty(dataRoot))p.Arguments+=" \""+dataRoot+"\"";process=Process.Start(p);process.ErrorDataReceived+=(s,e)=>{if(!string.IsNullOrEmpty(e.Data))try{File.AppendAllText(System.IO.Path.Combine(dataRoot??root,"rules-errors.log"),e.Data+Environment.NewLine);}catch{}};process.BeginErrorReadLine();}
 public async Task<J> Call(string command,object body=null){await gate.WaitAsync();try{if(process.HasExited)throw new Exception("规则进程已经关闭。请重新启动 AfterHours 恢复存档。");int id=++sequence;await process.StandardInput.WriteLineAsync(J.Json(J.O("id",id,"command",command,"body",body)));await process.StandardInput.FlushAsync();string line=await process.StandardOutput.ReadLineAsync();if(line==null)throw new Exception("规则进程意外退出，已完成的操作已保存。");J result=J.Parse(line);if(result["id"].I!=id)throw new Exception("内部消息顺序不一致");if(!result["error"].Null)throw new Exception(result["error"].S);return result["value"];}finally{gate.Release();}}
 public void Dispose(){try{process.StandardInput.Close();if(!process.WaitForExit(2500))process.Kill();process.Dispose();}catch{}}
}
