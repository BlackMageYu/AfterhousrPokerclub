...
async function requestMobileAI(config,observation){
  const started=Date.now(),controller=new AbortController(),timeout=setTimeout(()=>controller.abort(),Math.max(1000,Number(config.timeoutSeconds??30)*1000));
  try{
  ...
  }catch(error){if(error?.name==='AbortError')throw new Error('外部 AI 等待超时');throw error;}finally{clearTimeout(timeout);}
}
...
export class MobileGameStore {
  constructor({random=Math.random}={}) {
    ...
    this.ai={mode:'local',baseUrl:'',model:'',timeoutSeconds:30,apiKey:''};
    this.reports=readJSON(REPORTS_KEY,{});
    const saved=readJSON(STORAGE_KEY,null);
    if(saved){
      if(saved.profile)this.profile=saved.profile;
      this.lastSummary=saved.lastSummary??null;this.discovery=saved.discovery??this.discovery;this.world=saved.world??this.world;this.ai={...this.ai,...(saved.ai??{})};this.careerStats=saved.careerStats??{};this.memoryBank=saved.memoryBank??{};
  ...
  async externalBotTick(body){
    ...
    const observation=botObservation(e,e.hand.actor),started=Date.now();let decision,metadata;
    try{const result=await requestMobileAI(this.ai,observation);decision=result.decision;metadata={source:'external',model:this.ai.model,elapsedMs:result.elapsedMs,usage:result.usage};this.ai.lastSource='external';this.ai.lastError='';}
    catch(error){decision=chooseBotAction(observation,this.random);metadata={source:'fallback',model:this.ai.model,elapsedMs:Date.now()-started,errorCode:'external_error',error:error.message};this.ai.lastSource='fallback';this.ai.lastError=error.message;}
  ...
  dispatch(route,body){
    ...
    if(name==='ai-settings'){if(this.session&&body.mode&&body.mode!==this.ai.mode)throw new Error('请先下桌再切换 AI 模式');this.ai={...this.ai,...body,timeoutSeconds:Number(body.timeoutSeconds??30)};this.save();return this.state();}
